// finansle-backend.js — loads data/daily.json and renders a big, reliable chart
class FinansleGame {
  constructor() {
  ...
  async init() {
    try {
      ...
      await this.loadDailyJson();          // fetch today's data
      this.setupEventListeners();          // wire UI

      this.showMainChart();                // Financhle-style hint 1: big 5Y chart
      this.positionCluesAboveSearch();     // better UX
  ...
  async loadDailyJson() {
    ...
    const { stock, allStocks } = this.normalizeDailyJson(raw);
  ...
  normalizeDailyJson(raw) {
    ...
    if (isFlat) {
      ...
      const allStocks =
        raw.all_stocks?.map((x) => ({
          name: x.name || x.company_name || x.ticker || x.symbol || "",
          ticker: (x.symbol || x.ticker || "").toUpperCase(),
          english_name: x.name || x.company_name || "",
        })) || [];
      return { stock: s, allStocks };
    }

    // Nested example shape fallback
  ...
  applyDerivedMetrics(s) {
    ...
    const p2 = byYearsBack(2), p1 = byYearsBack(1);
    s.performance_2y = +pct(p2, last).toFixed(2);
    s.performance_1y = +pct(p1, last).toFixed(2);

    const oneYearAgo = new Date(data[data.length - 1].date); oneYearAgo.setFullYear(oneYearAgo.getFullYear() - 1);
  ...
  showAutocompleteOptions(options) {
    ...
    options.forEach(s => {
      ...
      const label = (s.english_name && s.english_name !== s.name)
        ? `${s.name} / ${s.english_name} (${s.ticker})`
        : `${s.name} (${s.ticker})`;
  ...
  handleGuess() {
    ...
    setTimeout(() => this.processGuess(guess), 250);
  }
  ...
  processGuess(guess) {
    try {
      ...
      const isCorrect =
        g.ticker?.toUpperCase() === this.dailyStock.ticker?.toUpperCase() ||
        g.name === this.dailyStock.company_name;
  ...
  showMainChart() {
    ...
    // height from CSS var (overridden in CSS overrides below)
    ...
    // draw AFTER layout so we get real size
  ...
  drawChartLine() {
    ...
    // reliable size for SVG (clientWidth/Height are often 0 for SVG)
    const rect = svg.getBoundingClientRect();
    const width  = Math.max(300, rect.width  || 0);
    const height = Math.max(150, rect.height || 0);

    if (width === 0 || height === 0) {
      // try next frame if layout isn't ready yet
      requestAnimationFrame(() => this.drawChartLine());
      return;
    }
    ...
    // clear old shapes (keep defs & grid rect)
    [...svg.querySelectorAll("polyline,polygon,circle")].forEach(n => n.remove());
